...
| /password | POST | Use this route to send a password reset confirmation email to users that registered by email. Accepts **`email`** as the only param. The user matching the `email` param will be sent instructions on how to reset their password. |
| /password | PATCH | Use this route to change users' passwords. Requires **`password`**, **`password_confirmation`** and **`reset_password_token`** as params. |
| /password/edit | GET | Verify user by password reset token. This route is the destination URL for password reset confirmation. This route must contain the **`reset_password_token`** param. This will be set automatically by the confirmation email that is generated by the password reset request. |
